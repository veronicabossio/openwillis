...
bkhealth_green = '#C9DFCA'
groupby = 'Grade Level'

def get_anova(df, groupby, metrics, group_order):    
    #grade_order = ['Kindergarten', 'Elementary', 'Teen', 'College', 'Adult']
    # Group the dataframe by grade for easy access
    grouped = df.groupby(groupby)

    anova_results = {}
    for metric in metrics:
        # Extract metric values for each grade level in order
        groups = [grouped.get_group(level)[metric].dropna() for level in group_order] # list of series
...
def make_box_trace(df, metric, group_order, groupby, stat=None, pval=None):
    """
    Returns a list of go.Box traces for each grade level.
    """
    #bkhealth_green = 'mediumseagreen'  # Replace with your actual 'bkhealth_green' color
    # Define a color for the box lines and outlier markers that is darker than the fill.
    darker_green_for_line = 'darkgreen'
    traces = []
    for grade in group_order:
        traces.append(go.Box(
            y=df[df[groupby] == grade][metric],
            name=grade,
            fillcolor=bkhealth_green,
            line=dict(
                color=darker_green_for_line,  # Use the defined darker color for the line
                width=1  # Slightly thicker line for better visibility
            ),
            marker=dict(
                color=darker_green_for_line,  # Use the defined darker color for outliers
                opacity=0.5  # Make outliers fully opaque
            ),
